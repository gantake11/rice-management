...
| :--- | :--- | :--- | :--- |
| **Phase 1: 要件定義** | [`/doc/01_requirements/`](./01_requirements/) | 「誰のどんな課題を解決するか」「何を作るか」の定義 | 進行中 (2/4 完了) |
...
│
├── 01_requirements/                   # 【Phase 1】要件定義（進行中: 2/4）
│   ├── 01_overview.md                 # [完了] プロジェクト概要・背景・解決する課題・目的
│   ├── 02_user_and_scope.md           # [完了] ターゲットユーザー・ペルソナ・スコープ(MVP/将来)
...
- **タスクリスト**:
  - [x] ターゲットユーザー（ペルソナ）の具体化
  - [x] 代表的なユースケース（ユーザー行動フロー）の策定
  - [x] MVPスコープ（初期リリース必須機能）の切り出し
  - [x] 将来機能（フェーズ2以降で対応するもの）のリストアップ
